/ChatCommand/NetDebug
	Command = "nets"
	MinPowerLevel = RankProgrammer

/ChatCommand/NetDebug/Execute(var/mob/Player, var/CommandText)
	SendUser("[Config.NetController.Networks.len] Networks")
	for(var/Index = 1; Index <= Config.NetController.Networks.len; Index++)
		var/datum/Network/Network = Config.NetController.Networks[Index]
		if (Network)
			SendUser("\white Network [Index]:\yellow [Network.Cables.len] [Network.Type] cable with [Network.Devices.len] devices")
		else
			SendUser("\white Network [Index]:\red Removed")
	return

/ChatCommand/ShowNets
	Command = "shownets"
	MinPowerLevel = RankProgrammer

/ChatCommand/ShowNets/Execute(var/mob/Player, var/CommandText)
	for(var/obj/MapMarker/Network/N in world)
		N.invisibility = Visible